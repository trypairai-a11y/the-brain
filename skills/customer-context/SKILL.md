...
1. Look up the customer by phone (the external_id convention):
...
404 → new customer; skip this skill, and `file-conversation` will create them
when the thread ends. 2. The response carries `data` (name, phone, notes), `links` (their
...
your tone; a recent `renewed` is worth acknowledging. 3. For depth, walk the graph or timeline:
